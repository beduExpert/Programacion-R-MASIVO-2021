...
p <- p +
  ...
  annotate("text", x = mex$Fecha[round(dim(mex)[1]*0.4)], 
           y = max(mex$Infectados)-150000, colour = "red", size = 5, label = paste("Última actualización para Muertes:", mex$Decesos[dim(mex)[1]])) 
p
...
p <- p +
  theme(plot.margin=margin(10,10,20,10), plot.caption=element_text(hjust=1.05, size=10)) +
  annotate("text", x = mexm$Fecha[round(length(mexm$Fecha)*0.5)], 
